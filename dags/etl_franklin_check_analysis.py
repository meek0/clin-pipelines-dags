...
with DAG(
    ...
    with TaskGroup(group_id='franklin') as franklin:
        ...
        def _start_analysis(obj, token):
            ...
            batch = batch_id()
            for family_id, analyses in _families.items():
                started_analyses['families'][family_id] = start_analysis(family_id, analyses, token, clin_s3, batch)
    ...
    def check_franklin_needed():
        metadata_path = f'{batch_id()}/metadata.json'
        f = clin_s3.get_key(metadata_path, 'clin-local')
        file_content = json.loads(f.get()['Body'].read().decode('utf-8'))
        if 'CQGC_Germaline' in file_content['submissionSchema']:
            return True
        else:
            return False
